...
/* Search and compare up array until element tri-pair equals 2020.
 * Return multiplication of the three elements.
 */
int find_tri_pair(int *rep, int f_len)
{
	for (int i = 0; i < f_len-2; i++) {
		for (int j = i+1; j < f_len-1; j++) {
			for (int k = j+1; k < f_len; k++) {
				if (rep[i]+rep[j]+rep[k] == 2020) {
					return rep[i] * rep[j] * rep[k];
				}
			}
		}
	}
...
int main(void)
{
	...
	pair_multi = find_tri_pair(rep_array, f_len);

	if ( pair_multi )
		printf("Found tri-pair multiplication: %i\n", pair_multi);
	else 
		printf("Failed to accurately find tri-pair\n");
